...
export default class paidinvoicesComponent {
  /*@ngInject*/
  constructor($http, $scope, Auth, $uibModal, ) {
    ...
    // this.$http.get(properties.invoice_paid).success(function (response) { 

    //  $scope.Profile = response;

    //  });
    this.$http.get(properties.invoice_paid)
      .then(response => {
  ...
  openComponentModal() {
    var modalInstance = this.$uibModal.open({
      ...
      template: require('./Report/paidinformation.html'),
      scope: this.$scope,
      size: 'lg'
    });
